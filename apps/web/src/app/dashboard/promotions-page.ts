  ...
  template: `
    <div class="promotions-page">
      ...
      <p class="page-description">
        {{ (promotions$ | async)?.length }} promociones · filtro por estado, ciudad y umbral de margen
      </p>
      ...
      <div *ngIf="(promotions$ | async) === null" class="skeleton-loader">
        ...
        <div class="table-header-skeleton">
          <div *ngFor="let i of [1,2,3,4,5,6,7,8,9]" class="skeleton" style="width: 100%; height: 16px;"></div>
        </div>

        <!-- Table rows skeleton -->
        <div class="table-body-skeleton">
          <div *ngFor="let i of [1,2,3,4,5]" class="table-row-skeleton">
            <div *ngFor="let j of [1,2,3,4,5,6,7,8,9]" class="skeleton" style="width: 100%; height: 16px;"></div>
          </div>
        </div>
      </div>
      ...
      <div *ngIf="(promotions$ | async) !== null" class="promotions-content">
        ...
        <div class="toolbar filters-toolbar">
          ...
          <button 
            class="btn"
            *appHasRole="['Admin','Analyst']"
            (click)="navigateToNewPromotion()"
          >
            + Nueva promoción
          </button>
        </div>
        ...
        <div class="table-wrap">
          <table class="data-table">
              ...
              <tr>
                <th style="width: 40px;">
                  <input 
                    type="checkbox" 
                    id="select-all"
                    [checked]="selectedIds.size > 0 && selectedIds.size === (filteredPromotions$ | async)?.length"
            ...
            <tbody>
              <ng-container *ngIf="(filteredPromotions$ | async) as filteredPromotions; else emptyState">
                <ng-container *ngIf="filteredPromotions.length > 0; else emptyState">
                  <tr *ngFor="let promo of filteredPromotions" 
                    ...
                    <td class="t-num">
                      <span class="mini-bar">
                        <span 
                          [style.width.%]="_getSoldPercentage(promo)"
                        ></span>
                      </span>
                      {{ promo.unitsSold ?? 0 }}/{{ promo.unitsTotal }}
                    </td>
                    <td class="t-num">
                      {{ (promo.financials?.revenue_contracted_eur ?? 0) | currency:'EUR':'symbol-narrow':'1.0-0':'es-ES' }}
                    </td>
                    <td class="t-num">
                      <span class="pill"
                        [class.amber]="getPromotionStatus(promo) === 'amber'"
                    ...
                    <td *appHasRole="['Admin','Analyst']">
                      <button 
                        class="btn ghost"
                        (click)="navigateToPromotionDetail(promo.id)"
                      >
                        Detalle
                      </button>
                    </td>
                ...
                <tr>
                  <td colspan="9" class="empty-state">
                    <div class="empty">
                      ...
                      <div class="sub">
                        Ajuste los filtros o reduzca el umbral de margen.
                      </div>
                    </div>
                  </td>
                </tr>
