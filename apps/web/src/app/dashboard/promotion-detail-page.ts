...
@Component({
  ...
  template: `
    <div class="promotion-detail-page">
      <div class="detail-grid">
        ...
        <div>
          ...
          <div class="detail-hero" *ngIf="promotion$ | async as promotion; else loadingHero">
            ...
            <div class="loc">{{ promotion.location.city }}, {{ promotion.location.province }} · {{ promotion.unitsTotal }} unidades</div>
          </div>
          ...
          <ng-container *ngIf="promotion$ | async as promotion">
            ...
            <div class="detail-kpi">
              ...
              <div class="value">{{ (promotion.financials?.revenue_contracted_eur ?? 0) | currency:'EUR':'symbol-narrow':'1.0-0':'es-ES' }}</div>
              <div class="sub">Contracted</div>
            </div>
            <div class="detail-kpi">
              <div class="label">Margen bruto</div>
              <div class="value">{{ (promotion.financials?.projected_margin_eur ?? 0) | currency:'EUR':'symbol-narrow':'1.0-0':'es-ES' }}</div>
              <div class="sub">{{ (promotion.financials?.projected_margin_pct ?? 0) | number:'1.0-1' }}%</div>
            </div>
            <div class="detail-kpi">
              <div class="label">Margen bruto (%)</div>
              <div class="value">{{ (promotion.financials?.projected_margin_pct ?? 0) | number:'1.0-1' }}%</div>
              <div class="sub">{{ (promotion.financials?.projected_margin_eur ?? 0) | currency:'EUR':'symbol-narrow':'1.0-0':'es-ES' }}</div>
            </div>
            <div class="detail-kpi">
                            <div class="label">Pre‑ventas (%)</div>
                            <div class="value">{{ (promotion.unitsTotal > 0 ? ((promotion.unitsSold ?? 0) / promotion.unitsTotal * 100) : 0) | number:'1.0-1' }}</div>
                            <div class="sub">{{ promotion.unitsSold ?? 0 }} de {{ promotion.unitsTotal }} uds</div>
                          </div>
                          </div>
                        </ng-container>
                      </ng-container>
          ...
          <div class="section-block" *ngIf="promotion$ | async as promotion; else loadingEconomy">
            ...
            <div class="fin-stats">
              <div class="fin-stat">
                ...
                <div class="v">{{ (promotion.financials?.total_budget_eur ?? 0) | currency:'EUR':'symbol':'1.0-0':'es-ES' }}</div>
              </div>
              <div class="fin-stat">
                <div class="l">Gastos ejecutados</div>
                <div class="v">{{ (promotion.financials?.construction_spent_eur ?? 0) | currency:'EUR':'symbol':'1.0-0':'es-ES' }}</div>
              </div>
              <div class="fin-stat">
                <div class="l">Financiación dispuesta</div>
                <div class="v">{{ (promotion.financing?.drawn_eur ?? 0) | currency:'EUR':'symbol':'1.0-0':'es-ES' }} / {{ (promotion.financing?.loan_amount_eur ?? 0) | currency:'EUR':'symbol':'1.0-0':'es-ES' }}</div>
              </div>
              <div class="fin-stat">
                <div class="l">Gastos de suelo</div>
                <div class="v">{{ (promotion.financials?.land_cost_eur ?? 0) | currency:'EUR':'symbol':'1.0-0':'es-ES' }}</div>
              </div>
            </div>
            <div class="progress">
          ...
          <div class="section-block" *ngIf="promotion$ | async as promotion; else loadingSales">
            ...
            <div class="fin-stats">
              ...
              <div class="fin-stat">
                ...
                <div class="v">{{ (promotion.financials?.avg_price_m2_eur ?? 0) | currency:'EUR':'symbol-narrow':'1.0-0':'es-ES' }}</div>
              </div>
              <div class="fin-stat">
      ...
      <!-- Loading states -->
      <div *ngIf="(promotion$ | async) === null || (viability$ | async) === null" class="loading-container">
  ...
  styles: [`
    ...
    .risk-item .sev.alto  { background-color: rgba(185, 28, 28, 0.12);  color: var(--red); }
    .risk-item .sev.medio { background-color: rgba(180, 83, 9, 0.12);  color: var(--amber); }
    .risk-item .sev.bajo  { background-color: rgba(15, 118, 110, 0.12); color: var(--green); }
    .path-list {
      display: flex;
      flex-direction: column;
      gap: 0.5rem;
    ...
    .path-item.ok .dot          { background-color: var(--green); }
    .path-item.en-curso .dot    { background-color: var(--amber); }
    .path-item.pendiente .dot   { background-color: var(--subtle); }
    .path-item .when {
      font-size: 0.75rem;
      color: var(--muted);
      margin-left: auto;
...
export class PromotionDetailPageComponent {
  ...
  // Helper methods
  private isStepObject(step: CriticalPathStep): step is { text: string; when: string; status: string } {
  ...
  getViabilityStatus(promotion: Promotion, viability: ViabilityResult | null): 'green' | 'amber' | 'red' {
    if (!viability) return 'red'; // No viability data -> red
    
    const marginPct = viability.marginBrutoPct;
    const preVentasPct = promotion.unitsTotal > 0 ? (promotion.unitsSold ?? 0) / promotion.unitsTotal * 100 : 0;
  ...
  getBudgetExecutionPercent(promotion: Promotion): number {
    if (!promotion.financials?.construction_budget_eur || promotion.financials.construction_budget_eur === 0) {
      return 0;
    }
